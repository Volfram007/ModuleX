...
async def cmd_all(message: Message, state: FSMContext) -> None:
    if message.text.lower().strip() == "Рассчитать".lower():
        ...
        bot_mess = await message.answer(f"Какой Ваш пол: М или Ж ?",
                                        reply_markup=ReplyKeyboardRemove())
        await set_id(state, message, bot_mess)
    elif message.text.lower().strip() == "Информация".lower():
        await state.set_state(None)
        bot_mess = await message.answer(
...
#
# 1. Измените `message_handler` для функции `set_age`:
#     - Теперь он реагирует на текст 'Рассчитать' вместо 'Calories'.
#
# 2. Создайте клавиатуру `ReplyKeyboardMarkup` с двумя кнопками `KeyboardButton`:
#     - Текст кнопок: 'Рассчитать' и 'Информация'.
#     - Используйте параметр `resize_keyboard` для подстройки клавиатуры под размеры интерфейса устройства.
#
# 3. Используйте созданную клавиатуру в ответе функции `start` через параметр `reply_markup`.
#
# 4. При команде `/start` должна присылаться клавиатура с двумя кнопками:
#     - Нажатие на кнопку 'Рассчитать' запускает функцию `set_age`, с которой начинается работа машины состояний
