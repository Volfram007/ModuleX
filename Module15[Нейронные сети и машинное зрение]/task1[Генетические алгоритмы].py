...
GENES = '''абвгдеёжзийклмнопрстуфхцчшщъыьэюя
АБВГЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ 1234567890, .-;:_!"#%&/()=?@${[]}'''

# Целевая строка для генерации
# TARGET = """I remember a wonderful moment: You appeared in front of me"""
TARGET = ("""Я помню чудное мгновенье
Передо мной явилась ты""")


def highlight(gen: int, target: str, candidate: str) -> str:
    """
    Возвращает строку, в которой символы, совпадающие с целевой строкой, подсвечены зелёным цветом
    """
    highlight_text = ""
    green = "\033[92m"  # Код зелёного цвета
    reset = "\033[0m"  # Сброс цвета

    for t_char, c_char in zip(target, candidate):
        if t_char == c_char:
            highlight_text += f"{green}{c_char}{reset}"
        else:
            highlight_text += c_char
...
def main():
    ...
    while not found:
        ...
        # Определяем 10% популяции, наиболее соответствующих целевой функции,
        ...
        print(highlight(generation, TARGET, "".join(population[0].chromosome)),
